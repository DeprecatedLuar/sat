...
internal/
  config/
    config.go              # ~/.config/sat/config.toml (install fallback order), pregenerated by selfheal
  manifest/
  ...
  sources/
    ...
    github.go              # GitHub orchestrator: sequences github/ mechanics + AppImageInstall, no install logic of its own
    github/                # GitHub mechanics: API fetch, tree/release inspection, huber/go/python install, fuzzy search+disambiguation
...
- Install fallback order is user-configurable via `~/.config/sat/config.toml` (`internal/config`, using `BurntSushi/toml`), pregenerated with defaults by `selfheal.Run()` — unlike bash's hardcoded `INSTALL_ORDER`. Sources omitted from the array are skipped entirely, not just deprioritized.
...
**Go-only config** (no bash equivalent): `~/.config/sat/config.toml` (respecting `XDG_CONFIG_HOME`, same convention as bash's `~/.config/sat/exclude`) holds the install fallback order as a TOML array under `order`, pregenerated by `selfheal.Run()`. Sources not listed are never tried.
